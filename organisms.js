    ...
    for (var y = 0; y < height; y++) {
        ...
        for (var x = 0; x < width; x++) {
            if (map[y][x] !== 0) {
                ...
                let chosen = map[y][x]
                let accept = true
                let currSelection = selections[currentSelection](chosen.yPos, chosen.xPos)
                if (currSelection){
                    accept = true
                } else {
                    accept = false
...
function massSimulate() {
    if (!simulated && runFullDuration && started) {
        let currTick = 0
    ...
    else if (!simulated && !runFullDuration && started) {
        simulate(true)
    }
}
function guys(x) {
...
function changeCriteria(x) {
    currentSelection = x
    document.getElementById("selectionCriteria").innerHTML = "Selection Criteria: " + currentSelection
}
function genes(x) {
